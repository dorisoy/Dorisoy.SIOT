...
{
    public class SpeederView : SKCanvasView
    {
        /// <summary>
        /// 定义最大速度180
        /// </summary>
        ...
        {
            // 当 Speed 发生改变时，重绘 SpeederView
            ((SpeederView)bindable).InvalidateSurface();
        }
        ...
        private void DrawTicksAndLabels(SKCanvas canvas, float centerX, float centerY, float radius)
        {
            ...
            for (int i = 0; i <= totalTicks; i++)
            {
                ...
                if (isMajorTick)
                {
                    float labelRadius = radius - 60;
                    int speed = ((i / (10)) * MajorTickInterval) / 10 * 10;
        ...
        private void DrawNeedle(SKCanvas canvas, float centerX, float centerY, float needleLength, float speed)
        {
            ...
            using (var needlePaint = new SKPaint())
            {
                ...
                // 绘制指针底部固定的圆 f2f5fc
                // 圆的半径，可根据实际需要调整
                float circleRadius = NeedleWidth + 240;
                needlePaint.Style = SKPaintStyle.Fill;
                needlePaint.Color = SKColor.Parse("#d0d7e6");
